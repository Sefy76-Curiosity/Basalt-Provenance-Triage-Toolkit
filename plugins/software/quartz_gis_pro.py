"""
Quartz GIS v1.3.0 - Ultimate Archaeologist's Toolkit
...
✓ ALL v1.2.2 features polished and perfected
"""
...
PLUGIN_INFO = {
    ...
    "id": "quartz_gis",
    "name": "Quartz GIS",
    "description": "Ultimate GIS for archaeologists - Measure, viewshed, QGIS export!",
    "icon": "🌍",
    "version": "1.3.0",
    "requires": [
...
class ViewshedCalculator:
    ...
    @staticmethod
    def calculate(dem, observer_x, observer_y, observer_height=1.7, max_distance=5000):
        """
        ...
        dz = dem - obs_z
        angles = np.arctan2(dz, distances * dem_resolution)  # Need DEM resolution

        # For each pixel, check if any pixel between it and observer blocks view
        # This is a simplified R2 algorithm
        visible = np.ones((rows, cols), dtype=bool)
        ...
        for idx in sort_idx:
            ...
            for px, py in line_pixels:
                ...
                angle = np.arctan2(z_diff, d * dem_resolution)
...
class QuartzGISPlugin:
    ...
    def _calculate_viewshed(self, sample):
        ...
        try:
            ...
            visible = ViewshedCalculator.calculate(
                dem_data, col, row,
                observer_height=1.7,  # Average person height
                max_distance=5000      # 5km visibility
            )
    ...
    def open_window(self):
        ...
        self.window.title("🌍 Quartz GIS v1.3.0 - Ultimate Archaeologist's Toolkit")
    ...
    def _create_interface(self):
        ...
        tk.Label(header, text="v1.3.0 - Ultimate Archaeologist's Toolkit",
                font=("Arial", 10),
    ...
    def _create_right_panel(self, parent):
        ...
        self._create_attribute_table(table_tab)

    def _create_attribute_table(self, parent):
        """Create functional attribute table"""

        # Controls
        ctrl_frame = tk.Frame(parent, bg="#f5f5f5", height=40)
        ctrl_frame.pack(fill=tk.X)
        ctrl_frame.pack_propagate(False)

        tk.Button(ctrl_frame, text="🔄 Refresh",
                 command=self._refresh_attribute_table).pack(side=tk.LEFT, padx=5)

        tk.Button(ctrl_frame, text="📥 Export to Dynamic Table",
                 command=self._export_to_dynamic_table).pack(side=tk.LEFT, padx=5)

        # Filter
        tk.Label(ctrl_frame, text="Filter:", bg="#f5f5f5").pack(side=tk.LEFT, padx=(20,5))
        self.filter_var = tk.StringVar()
        filter_entry = tk.Entry(ctrl_frame, textvariable=self.filter_var, width=20)
        filter_entry.pack(side=tk.LEFT, padx=5)
        filter_entry.bind('<Return>', lambda e: self._filter_table())

        tk.Button(ctrl_frame, text="Apply", command=self._filter_table,
                 bg="#3498db", fg="white").pack(side=tk.LEFT, padx=5)

        tk.Button(ctrl_frame, text="Clear", command=self._refresh_attribute_table,
        ...
        self.attr_table = ttk.Treeview(table_frame,
                                       yscrollcommand=vsb.set,
                                       xscrollcommand=hsb.set,
                                       selectmode="extended")

        vsb.config(command=self.attr_table.yview)
        hsb.config(command=self.attr_table.xview)

        self.attr_table.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
    ...
    def _refresh_attribute_table(self):
        ...
        for row in self.attr_table.get_children():
            self.attr_table.delete(row)

        columns = list(gdf.columns)
        self.attr_table["columns"] = columns

        for col in columns:
            self.attr_table.heading(col, text=col)
            self.attr_table.column(col, width=100)

        for idx, row in gdf.iterrows():
            values = [str(row[col])[:50] for col in columns]
            self.attr_table.insert("", "end", values=values)
    ...
    def _filter_table(self):
        ...
        for idx, row in gdf.iterrows():
            match = False
            for col in columns:
                if filter_text in str(row[col]).lower():
                    match = True
                    break
...
def setup_plugin(main_app):
    ...
    print(f"🌍 Quartz GIS v1.3.0 loading...")
