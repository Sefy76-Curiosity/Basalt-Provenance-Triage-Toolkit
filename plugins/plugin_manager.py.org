...
class PluginManager(ttk.Toplevel):
    # --- Constants ---
    WINDOW_SIZE = "620x500"
    MIN_SIZE = (580, 450)

    # Cache fonts to avoid dict lookups
    HEADER_FONT = ("TkDefaultFont", 12, "bold")
    TAB_FONT = ("TkDefaultFont", 11, "bold")
    PLUGIN_TITLE_FONT = ("TkDefaultFont", 10, "bold")
    PLUGIN_DESC_FONT = ("TkDefaultFont", 8)
    PLUGIN_META_FONT = ("TkDefaultFont", 7)
    BUTTON_FONT = ("TkDefaultFont", 8, "bold")
    STATUS_FONT = ("TkDefaultFont", 9)
    ...
    def __init__(self, parent_app):
        ...
        self.colors = self.ttk_style.colors
    ...
    def _check_deps(self, requires_tuple: Tuple[str]) -> Tuple[bool, Tuple[str]]:
        """Check dependencies with caching for performance."""
        if not requires_tuple:
            return True, ()
        missing = []
        for pkg in requires_tuple:
            if pkg == 'ctypes':
                continue
    ...
    def _check_deps_list(self, requires: List[str]) -> Tuple[bool, List[str]]:
        """Wrapper for list input."""
        if not requires:
            return True, []
        result, missing_tuple = self._check_deps(tuple(requires))
        return result, list(missing_tuple)
    ...
    def _install_deps(self, plugin_name: str, packages: List[str]):
        ...
        header = ttk.Frame(win, bootstyle="primary")
        header.pack(fill=tk.X)
        ttk.Label(header, text=f"pip install {' '.join(packages)}",
                 font=("Consolas", 9)).pack(pady=6)

        text = tk.Text(win, wrap=tk.WORD, font=("Consolas", 9))
        scroll = ttk.Scrollbar(win, command=text.yview, bootstyle="dark-round")
    ...
    def _build_ui(self):
        ...
        # Get the actual background color from the frame
        header.update_idletasks()
        bg_color = header.cget("background")

        label1 = tk.Label(header, text="🔌", font=("TkDefaultFont", 16),
                        bg=bg_color, fg="white")
        label1.pack(side=tk.LEFT, padx=10)

        label2 = tk.Label(header, text="PLUGIN MANAGER v2.0", font=self.HEADER_FONT,
                        bg=bg_color, fg="white")
        ...
        self.frames = {
            "add-ons": ttk.Frame(self.content_frame),
            "software": ttk.Frame(self.content_frame),
            "hardware": ttk.Frame(self.content_frame)
        }
        ...
        status = ttk.Label(left_frame, textvariable=self.status_var,
                        font=self.STATUS_FONT)
        status.pack(side=tk.LEFT, padx=15)

        # Source indicator
        self.source_var = tk.StringVar(value="")
        source_label = ttk.Label(left_frame, textvariable=self.source_var,
                                font=("TkDefaultFont", 8))
        source_label.pack(side=tk.LEFT, padx=5)

        # Right side - action button
        self.action_btn = ttk.Button(footer, text="✅ APPLY CHANGES",
                                    bootstyle="success",
                                    command=self._apply)
    ...
    def _switch_category(self, category: str):
        ...
        self.btn_addons.config(bootstyle="primary" if category == "add-ons" else "secondary")
        self.btn_software.config(bootstyle="primary" if category == "software" else "secondary")
        self.btn_hardware.config(bootstyle="primary" if category == "hardware" else "secondary")
        ...
        for cat, frame in self.frames.items():
            if cat == category:
                ...
                loading = ttk.Label(frame, text="Loading plugins...", font=self.STATUS_FONT)
                loading.pack(expand=True)
                frame.pack(fill=tk.BOTH, expand=True)
                # Populate after a brief pause to show loading
                self.after(10, lambda f=frame, c=category, l=loading: self._populate_merged_category(f, c, l))
            else:
                frame.pack_forget()
    ...
    def _populate_merged_category(self, parent, category: str, loading_label=None):
        ...
        try:
            ...
            if not merged:
                empty = ttk.Frame(parent)
                empty.pack(fill=tk.BOTH, expand=True)
                ttk.Label(empty, text="✨ No plugins found",
                         font=("TkDefaultFont", 11)).pack(expand=True)
                return

            # Canvas + Scrollbar
            canvas = tk.Canvas(parent, highlightthickness=0, bg=self.colors.bg)
            ...
            # Create rows - use update_idletasks periodically to show progress
            for i, plugin in enumerate(merged):
                self._create_unified_row(scroll_frame, plugin)
                # Update every 10 rows to show progress
                if i % 10 == 0:
    ...
    def _create_unified_row(self, parent, plugin: Dict):
        ...
        # Main row frame - with subtle border for visual separation
        row = ttk.Frame(parent, relief="solid", borderwidth=1)
        ...
        ttk.Label(title_frame, text=title_text, font=self.PLUGIN_TITLE_FONT,
                  bootstyle="inverse").pack(anchor="w")

        # Description - keep wraplength as requested
        desc_label = ttk.Label(content, text=description,
                              font=self.PLUGIN_DESC_FONT,
                              bootstyle="light", wraplength=350)
        desc_label.pack(anchor="w", pady=(2,0))

        # Author
        ttk.Label(content, text=f"by {author}", font=self.PLUGIN_META_FONT,
        ...
        # Dependency status
        if requires:
            met, missing = self._check_deps_list(requires)
            if not met:
                deps_btn = ttk.Button(actions, text="⚠️ Deps",
                                     bootstyle="warning",
                                     command=lambda n=name, m=missing: self._install_deps(n, m))
        ...
        if local_info and not is_broken:
            ...
            cb = ttk.Checkbutton(actions, text="Enable", variable=var,
                                bootstyle="primary")
            ...
            ttk.Label(actions, text="⚠️ Broken", font=self.BUTTON_FONT,
                     bootstyle="danger").pack(side=tk.TOP, pady=1)
            uninstall_btn = ttk.Button(actions, text="🗑️ Uninstall",
                                      bootstyle="danger",
                                      command=lambda p=pid, i=local_info: self._uninstall_plugin(p, i))
            uninstall_btn.pack(side=tk.TOP, pady=1, fill=tk.X)
            ...
            install_btn = ttk.Button(actions, text="INSTALL",
                                    bootstyle="info",
    ...
    def _install_from_store(self, info: Dict, update: bool = False):
        ...
        progress = ttk.Toplevel(self)
        progress.title(f"{action} {info.get('name', pid)}")
        progress.geometry("400x150")
        progress.transient(self)
        ttk.Label(progress, text=f"{action} {info.get('name', pid)}...",
                 font=("TkDefaultFont", 10)).pack(pady=10)
        status = ttk.Label(progress, text="", font=("TkDefaultFont", 8))
        status.pack()
    ...
    def _update_button_state(self):
        ...
        if to_enable > 0 and to_disable > 0:
            self.action_btn.config(text=f"⚡ APPLY ({to_enable}/{to_disable})", bootstyle="warning")
        elif to_enable > 0:
            self.action_btn.config(text=f"✅ ENABLE {to_enable}", bootstyle="success")
        elif to_disable > 0:
            self.action_btn.config(text=f"🔥 DISABLE {to_disable}", bootstyle="danger")
        else:
            self.action_btn.config(text="✅ APPLY", bootstyle="success")
    ...
    def _apply(self):
        ...
        if plugins_to_enable:
            ...
            for pid in plugins_to_enable:
                ...
                if info and info.get('requires'):
                    met, missing = self._check_deps_list(info['requires'])
                    if not met:
        ...
        if changes > 0:
            ...
            self.action_btn.config(text="✅ DONE", bootstyle="success")
